...
import { makeupData } from '../utli/objectData/makeupdata';
...
function Makeup() {
  ...
  // Pagination logic
  const totalPages = Math.ceil(makeupData.length / PRODUCTS_PER_PAGE);
  const start = (currentPage - 1) * PRODUCTS_PER_PAGE;
  const end = start + PRODUCTS_PER_PAGE;
  const sortedProducts = sortProducts(makeupData, sortBy);
  ...
  return (
    <div className='flex flex-col items-center m-5'>
     <div
                ...
                style={{ backgroundImage: `url(${bannar8})` }}
              >
             <h4 className='text-md font-serif animate-slideDownFade'>BeBold</h4>
             <h1 className='text-3xl font-serif font-extrabold animate-slideDownFade delay-300'>Makeup</h1>
           </div>
      <div className="p-4">
      <div className="flex justify-between items-center mt-8 mb-2">
        <span>
          Showing {start + 1}-{Math.min(end, makeupData.length)} of {makeupData.length} results
        </span>
