...
import { productData } from '../utli/objectData/productdata';
...
function ShopAll() {
  ...
  const sortedProducts = sortProducts(productData, sortBy);
...
const dispatch = useDispatch();
  ...
  return (
    <div className='flex flex-col items-center m-5'>
      <div
           className="w-full h-[20rem] bg-cover bg-center flex items-center px-10 rounded-xl flex-col justify-center text-gray-700"
           style={{ backgroundImage: `url(${bannar7})` }}
         >
        <h4 className='text-md font-serif animate-slideDownFade'>BeBold</h4>
        <h1 className='text-3xl font-serif font-extrabold animate-slideDownFade delay-300'>SHOP ALL</h1>
      </div>
      <div className="p-4">
      <div className="flex justify-between items-center mt-8 mb-2">
        <span>
          Showing {start + 1}-{Math.min(end, productData.length)} of {productData.length} results
        </span>
